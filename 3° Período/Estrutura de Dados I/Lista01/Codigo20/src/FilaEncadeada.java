...
public class FilaEncadeada implements Iterable<Integer> {
    ...
    public boolean estaEmOrdemCrescente() {
        if (this.isEmpty() || this.inicio.getProximo() == null) {
            return true;
        }

        No noAtual = this.inicio;
        int valorAnterior = noAtual.getDado();
        noAtual = noAtual.getProximo();

        while (noAtual != null) {
            int atualDado = noAtual.getDado();
            if (atualDado <= valorAnterior) {
                return false;
            }
            valorAnterior = atualDado;
            noAtual = noAtual.getProximo();
        }
