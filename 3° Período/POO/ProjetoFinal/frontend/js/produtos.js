    ...
    produtos.forEach((produto) => {
      ...
      tr.innerHTML = `
        ...
        <td style="display: flex; gap: 8px;">
          <button onclick="prepararEdicaoProduto(${produto.id})" class="btn btn-small btn-warning">Editar</button>
          <button onclick="deletarProduto(${produto.id}, ${produto.estoqueAtual})" class="btn btn-small btn-danger">Excluir</button>
        </td>
      `;
...
window.prepararEdicaoProduto = async function (id) {
  try {
    ...
    const selectFornecedor = document.getElementById("fornecedorSelect");
    if (produto.fornecedor) {
      selectFornecedor.value = produto.fornecedor.id;
    } else {
...
async function deletarProduto(id, estoqueAtual) {
  if (estoqueAtual !== 0) {
    alert(
      "Não é possível excluir este produto. O estoque atual é diferente de zero.",
    );
    return;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(
        "Não é possível excluir este produto. Ele possui registros vinculados a ele.",
      );
    }
  } catch (error) {
    console.error(error);
